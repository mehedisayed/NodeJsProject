...
var customerModel = require.main.require('./models/manager/customer-model');

router.get('/', function(req, res) {
    customerModel.getAll(function(results){
        if(results.length > 0){
            res.render('manager/customers/customers', {userlist: results});
        }else{
...
router.get('/edit/:id', function(req, res) {
    customerModel.getById(req.params.id, function(result){
        customerModel.getAllUserType(function(usertype)
         {
            res.render('manager/customers/edit_customers', {user: result,usertypelist:usertype});
         });
    });
...
router.post('/edit/:id', function(req, res) {
    ...
    customerModel.update(user, function(status){
        if(status){
            ...
            res.redirect('/manager/customers/edit/'+req.params.id);
        }
    });
});
...
router.get('/new', function(req, res) {
    customerModel.getAllUserType(function(results)
        {
           res.render('manager/customers/edit_customers', {user:'',usertypelist:results});
        });
});
router.post('/new', function(req, res) {
    ...
    customerModel.insert(user, function(status){
        if(status){
            ...
            res.redirect('/manager/customers/new');
        }
    });
});
